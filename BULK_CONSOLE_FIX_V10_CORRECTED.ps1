# Bulk Console.error/warn Fixer - Version 10 CORRECTED
# CLIENT: Paths are now INSIDE src/ (to ./shared which is at client/src/shared/)
# WORKER/API: Paths to PROJECT ROOT /shared/
...
Write-Host "BULK CONSOLE FIX V10 CORRECTED" -ForegroundColor Cyan
...
# CLIENT: All paths are relative to src/ (errorLogger is at client/src/shared/)
# WORKER/API: Paths are relative to PROJECT ROOT (/shared/)
function Get-ErrorLoggerImportPath {
    ...
    # CLIENT FILES: Calculate relative path from file to ./shared/errorLogger.js (all within src/)
    if ($filePath -match '^client/src/') {
        $pathAfterSrc = $filePath -replace '^client/src/', ''
        $parts = $pathAfterSrc -split '/'
        $dirCount = $parts.Count - 1  # -1 to exclude filename
        
        if ($dirCount -eq 0) {
            # In src/ directly: ./shared/errorLogger.js
            return './shared/errorLogger.js'
        } else {
            # Below src/: count ups needed to reach src/
            $ups = '../' * $dirCount
