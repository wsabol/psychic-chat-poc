# Bulk Console.error/warn Fixer - Version 9 FINAL
# CLIENT: Uses relative paths to ./shared (synced from /shared/errorLogger.js)
...
Write-Host "BULK CONSOLE FIX V9 FINAL" -ForegroundColor Cyan
...
# For client: Relative path to ./shared/errorLogger.js (within src/)
# For worker/api: Relative paths to PROJECT ROOT /shared/
function Get-ErrorLoggerImportPath {
    ...
    # CLIENT FILES: Count directory depth below src/, that's how many ../ needed
    if ($filePath -match '^client/src/') {
        ...
        if ($dirCount -eq 0) {
            ...
            # Below src/: count ups needed
            $ups = '../' * ($dirCount + 1)  # +1 for src itself
