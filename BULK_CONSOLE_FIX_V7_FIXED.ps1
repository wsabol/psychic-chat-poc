# Bulk Console.error/warn Fixer - Version 7 FIXED
...
# FIXED: Corrected depth calculation for client/worker (count dirs only, not filename)
...
Write-Host "BULK CONSOLE FIX V7 FIXED" -ForegroundColor Cyan
...
# FIXED: Count DIRECTORY levels only, not the filename
function Get-ErrorLoggerImportPath {
    ...
    # api/something/file.js -> count dirs after 'api/', add 1 more for root
    if ($filePath -match '^api/') {
        $pathAfterApi = $filePath -replace '^api/', ''
        # Split and remove filename (last part)
    ...
    # client/src/components/file.js -> count dirs after 'client/', add 1 for root
    if ($filePath -match '^client/') {
        $pathAfterClient = $filePath -replace '^client/', ''
        # Split and remove filename (last part)
        $parts = $pathAfterClient -split '/'
        $dirCount = $parts.Count - 1  # -1 to exclude filename
        $ups = '../' * ($dirCount + 1)  # +1 to go up one more level to project root
        return $ups + 'shared/errorLogger.js'
    }
    
    # worker/modules/file.js -> count dirs after 'worker/', add 1 for root
...
foreach ($filePath in $Files) {
    ...
    if (Test-Path $fullPath) {
        # Skip .jsx and .tsx files - Node.js doesn't support JSX natively
        # These will be validated by React when app starts
