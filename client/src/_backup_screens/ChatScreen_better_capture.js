...
export function ChatScreen({
    ...
    // CAPTURE: Store horoscope when it becomes available
    useEffect(() => {
        if (personalInfo.horoscope && isOnboardingFlow && auth.isTemporaryAccount) {
            setOnboardingHoroscope({
            ...
            // Save to sessionStorage as backup
            try {
                sessionStorage.setItem('onboarding_horoscope', JSON.stringify({
                    data: personalInfo.horoscope,
                    timestamp: new Date().toISOString()
                }));
            } catch (e) {
    ...
    // Pass captured data to handlers for use during account creation
    useEffect(() => {
        if (handlers.setOnboardingData) {
            handlers.setOnboardingData({
                firstMessage: onboardingFirstMessage,
                horoscope: onboardingHoroscope
            });
        }
    }, [onboardingFirstMessage, onboardingHoroscope, handlers]);
