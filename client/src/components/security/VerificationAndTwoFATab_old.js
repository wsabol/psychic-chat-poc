      ...
      <div style={{ padding: '1rem', backgroundColor: '#f9f9f9', borderRadius: '6px' }}>
        ...
        {!flow.editMode && !flow.showVerification && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
            ...
            {/* Recovery Email */}
            <div style={{ fontSize: '12px', padding: '0.4rem 0', borderBottom: '1px solid #eee', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span>
                <strong>Recovery Email:</strong> {flow.recoveryEmail || 'Not set'}
              </span>
              {flow.recoveryEmail && methods?.recoveryEmailVerified && (
                <span style={{ backgroundColor: '#e8f5e9', color: '#2e7d32', padding: '0.2rem 0.5rem', borderRadius: '3px', fontSize: '11px', fontWeight: '600' }}>
                  ✓ Verified
                </span>
              )}
            </div>
