...
/**
 * SunSignInfo - Displays detailed sun sign information
 */
export default function SunSignInfo({ sunSignData }) {
        ...
        <div className="info-item">
          ...
          <span>
            {sunSignData._englishRulingPlanet 
              ? sunSignData._englishRulingPlanet.split('/').map((p, i) => (
                  <span key={i}>
                    {i > 0 && ' / '} 
                    {t(`planets.${p.toLowerCase().trim()}`)}
                  </span>
                ))
