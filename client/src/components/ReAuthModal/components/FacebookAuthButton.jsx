import React from 'react';
import styles from '../ReAuthModal.module.css';

/**
 * FacebookAuthButton - Facebook Sign-in button icon
 * Extracted component for better organization
 * isLinked: true if Facebook is a linked provider on this Firebase account.
 *           false = grey out and disable (user didn't sign up with Facebook).
 */
export function FacebookAuthButton({ loading, onFacebookReAuth, t, isLinked = true }) {
  return (
    <button
      onClick={onFacebookReAuth}
      disabled={loading || !isLinked}
      className={styles.socialButton}
      title={isLinked ? t('security.reauth.facebookSignIn') : t('security.reauth.providerNotLinked', { provider: 'Facebook' })}
      style={!isLinked ? { opacity: 0.35, cursor: 'not-allowed', filter: 'grayscale(1)' } : undefined}
    >
      <svg viewBox="0 0 24 24" className={styles.socialIcon}>
        <path fill="#1877F2" d="M12 2.04C6.5 2.04 2 6.53 2 12.06C2 17.06 5.66 21.21 10.44 21.96V14.96H7.9V12.06H10.44V9.85C10.44 7.34 11.93 5.96 14.22 5.96C15.31 5.96 16.45 6.15 16.45 6.15V8.62H15.19C13.95 8.62 13.56 9.39 13.56 10.18V12.06H16.34L15.89 14.96H13.56V21.96A10 10 0 0 0 22 12.06C22 6.53 17.5 2.04 12 2.04Z"/>
      </svg>
    </button>
  );
}
