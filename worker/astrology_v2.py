...
def is_planet_retrograde(planet_id, jd):
    ...
    try:
        ...
        return planet_pos[3] < 0
    except:
        return False

def calculate_cosmic_weather_planets():
    """Calculate 10 planets with retrograde status and icons for cosmic weather."""
    try:
        now_utc = datetime.now(ZoneInfo("UTC"))
        jd = swe.julday(now_utc.year, now_utc.month, now_utc.day, now_utc.hour + now_utc.minute / 60.0 + now_utc.second / 3600.0)
        ...
        planet_data = []
        for planet_id, planet_name, planet_icon in planets:
            try:
                planet_pos = swe.calc_ut(jd, planet_id)
                ...
                print(f"Error calculating planet {planet_name}: {str(e)}", file=sys.stderr)
                continue
        
        return {"planets": planet_data, "timestamp": now_utc.isoformat(), "success": True}
    except Exception as e:
        print(f"Error calculating cosmic weather planets: {str(e)}", file=sys.stderr)
...
if __name__ == "__main__":
    ...
    elif request_type == "cosmic_weather_planets":
        result = calculate_cosmic_weather_planets()
    else:
