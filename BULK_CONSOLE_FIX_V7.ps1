# Bulk Console.error/warn Fixer - Version 7
# Process all files at once, then syntax check all, then report summary
# Works for api/, client/, worker/ with shared/errorLogger.js location
...
Write-Host "BULK CONSOLE FIX V7 - Batch Processing" -ForegroundColor Cyan
...
# Calculate relative path to errorLogger
...
function Get-ServiceName {
    ...
    $filename = $parts[-1] -replace '\.js(x|on)?$', ''
...
foreach ($filePath in $Files) {
    ...
    for ($i = 0; $i -lt $lines.Count; $i++) {
        ...
        # Replace console.error (simple regex, don't overthink it)
