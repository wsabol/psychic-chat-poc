 ...
 * Called by frontend after Firebase login before accessing chat
 */
router.post('/check-2fa/:userId', async (req, res) => {
  try {
    ...
    // Get user's email for sending code
    const userResult = await db.query(
    ...
    // Generate temporary token requiring 2FA
    const tempToken = generateToken(userId, true);
    ...
    console.error('[AUTH] Error checking 2FA:', error);
